...
namespace PathologicalGames
{
    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        #region Inspector 參數
        /// <summary>
        /// Returns the name of this pool used by PoolManager. 
        /// This will always be the same as the name in Unity, unless the name contains the work "Pool", 
        /// which PoolManager will strip out. 
        /// This is done so you can name a prefab or GameObject in a way that is development friendly. 
        /// For example, "EnemiesPool"  is easier to understand than just "Enemies" when looking through a project.
        /// </summary>
        [Rename("物件池名稱")]
        public string poolName = "";

        /// <summary>
        /// 勾選後實例化的遊戲對象的縮放比例將全是1，不勾選擇用Prefab默認的。
        /// </summary>
        [Tooltip("勾選後實例化的遊戲對象的縮放比例將全是1，不勾選擇用Prefab默認的")]
		...
		public void CreatePrefabPool(PrefabPool prefabPool)
		{
			...
			if (isAlreadyPool)
            	...
            	(
                    "Prefab '{0}' 已經在名為 '{1}' 的物件池(SpawnPool)裡了！ 同一個 Prefabs 可以在很多個物件池裡，但每個單獨的物件池裡的不能有重複的 Prefab！",
					prefabPool.prefab, 
					this.poolName
				));
    ...
    public class PrefabPool
    {
        ...
        /// <summary>
        /// 限制遊戲中允許的實例數量。
        ///	打勾(=true) 意味著當達到“限制數量”時，將不再創建實例
        /// CALLS TO SpawnPool.Spawn() WILL BE IGNORED, and return null!
        ///
        /// 這對於子彈或爆炸彈等非關鍵物體非常有用。
        ///	除非在你的遊戲開始時忽略敵人的生成，否則你永遠不會想啟用這個項目用於敵人之類的物件。
        /// </summary>
        [Rename("啟用[數量限制]")]
        [Tooltip("建議只用於非關鍵物體 (如：子彈、爆炸...)")]
        public bool limitInstances = false;

        /// <summary>
        /// 這是「limitInstances」打勾時允許的最大實例數。
        /// </summary>
        [Rename("- 最大數量 (優先度高於「預載數量」)")]
        public int limitAmount = 100;

        /// <summary>
        /// FIFO代表「先進先出」。
        /// 通常，限制物件數量後，會停止生成並返回null。
        /// 如果啟用此選項(設置為true)，則第一個生成的物件將被重新使用，
        /// 從而使總生成的數量受到限制但仍會生成新實例。
        /// 
        /// 像是：如果我們限制了緩存池裡面只能有10個Prefab，
        /// 如果不勾選它，那麼你拿第11個的時候就會返回null。
        /// 如果勾選它在取第11個的時候他會返回給你前10個里最不常用的那個。
        /// </summary>
        [Rename("- FIFO (先進先出)")]
        public bool limitFIFO = false;  // Keep after limitAmount for auto-inspector

        /// <summary>
        /// 如果池的大小過大，請使用此功能從池中刪除已失效(非活動)的實例。
        ///	
        /// 除非您碰上需要管理記憶體的問題，否則請勿使用此功能！
        /// 這通常僅用於碰上內存管理的極端情況。
        /// 
        /// 對於大多數的物件池 (or games for that matter)(括弧內的翻譯看不懂), 
        /// 最好將其關閉，因為內存比性能更有餘裕(?)。
        /// 如果你確實需要這個，你可以微調這個觸發目標極端事件的頻率。
        /// 
        /// 何時使用它的一個例子是：
        /// 如果你使用物件池子彈，而且通常一次不需要超過10個，
        /// 但是某次需要進行大規模的一次性滅火事件，需要用到50個射彈。
        /// 
        /// 不是將額外的40個保留在內存裡，
        /// 將「Cull Above」屬性設置為15（遠高於預期的最大值），
        /// 物件池將 Destroy() 額外的物件，以從遊戲中釋放內存。
        /// 
        /// 這不會立即完成，因為你不會希望這個剔除功能與Pool對抗，
        /// 導致在滅火事件仍在進行時，引起額外的 Instantiate() 和 Destroy() 調用。
        /// 有關如何對此進行微調的詳細信息，請參閱“剔除延遲”。
        /// </summary>
        [Rename("啟用[剔除功能] (智慧清理)")]
        public bool cullDespawned = false;

        /// <summary>
        /// 要保留的總數量 (spawned + despawned) 
        /// 緩存池自動清理，但是始終保留幾個對像不清理。
        /// </summary>
        [Rename("- 要保留的總數量(顯示+關閉的)")]
        public int cullAbove = 50;

        /// <summary>
        /// 當總數量(顯示+關閉的) 大於 cullAbove設定的數值時執行，
        /// 會產生一個自動計時器，直到總數量低於 cullAbove設定的數值，
        /// 這裡即是設定每過多久會執行一遍自動清理，單位是秒。
        /// </summary>
        [Rename("- 每幾秒自動清理一次?")]
        public int cullDelay = 60;

        /// <summary>
        /// 每次清理幾個遊戲對象，另外這裡的清理是指 Destroy()
        /// </summary>
        [Rename("- 每次自動清理幾個遊戲對象")]
        ...
        #region Pool 功能
        /// <summary>
        /// Is set to true when the culling coroutine is started so another one won't be
        /// </summary>
