...
public static class PGEditorUtils 
{
    ...
    public const int CONTROLS_DEFAULT_LABEL_WIDTH = 176;
    public const string FOLD_OUT_TOOL_TIP = "點擊 展開/摺疊";
    ...
    /// </summary>
    /// <param name="label"        > 列表名稱 </param>
    /// <param name="list"         > List 類型的參數 </param>
    /// <param name="expanded"     > 用於確定主要折疊狀態的bool </param>
    /// <param name="foldOutStates"> Dictionary <object，bool> 用於跟踪列表-細項的狀態 </param>
    /// <param name="collapseBools">
    /// 如果為true，則列表項上的bool將折疊其後的字段(?)
                                          ...
                                          where T : new()    
    {
        ...
        // Main Add button
        if (GUILayout.Button(new GUIContent("+", "點擊增加"), EditorStyles.toolbarButton)) {
            list.Add(new T());
        }
        EditorGUILayout.EndHorizontal();
        ...
        for (int i = 0; i < list.Count; i++)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            EditorGUI.indentLevel = 0;  // Space will handle this for the header
            GUILayout.Space((indent+3)*6); // Matches the content indent  //3是每個 Prefab 內縮的大小
            ...
            {
                EditorGUI.indentLevel = indent + 2; //2是每個 Prefab 參數內縮的大小
            ...
            #region 流程列表更改 (Process List Changes)
            // Don't allow 'up' presses for the first list item
            switch (listButtonPressed)
            {
                case LIST_BUTTONS.None: // Nothing was pressed, do nothing
    ...
    public static void SerializedObjectFields<T>(T instance, bool collapseBools)
    {
        ...
        // 動態顯示字段 (Display Fields Dynamically)
        foreach (System.Reflection.FieldInfo fieldInfo in fields)
        {
            if (!collapseBools)
            {
            ...
            // 如果這是一個布爾。 添加字段並將折疊設置為true直到結束或直到另一個bool被擊中
            // (If this is a bool. Add the field and set collapse to true until the end or until another bool is hit)
            if (fieldInfo.FieldType == typeof(bool))
            {
                // 如果此bool的前4個字母與最後一個字母匹配，請將其包含在折疊組中，而不是啟動新的字母。
                // (If the first 4 letters of this bool match the last one, include this in the collapse group, rather than starting a new one.)
    ...
    /// <summary>
    /// 1. 使用System.Reflection.FieldInfo添加字段 (Uses a System.Reflection.FieldInfo to add a field)
    /// 2. 處理大多數內置類型和組件 (Handles most built-in types and components)
    /// 3. 包括默認情況下Inspector所做的自動命名 (includes automatic naming like the inspector does)
    /// </summary>
    public static void FieldInfoField<T>(T instance, System.Reflection.FieldInfo fieldInfo)
    {
        ...
        //針對 RenameAttribute 的顯示名稱修正 ----------------------------------------------------------------
        //參考：https://blog.csdn.net/qq_17813937/article/details/73299132
        object[] objs = fieldInfo.GetCustomAttributes(typeof(RenameAttribute), true);
        if (objs != null && objs.Length > 0) {
            label = ((RenameAttribute)objs[0]).name;
        }


        #region 內置數據類型 (Built-in Data Types)
    ...
    private static LIST_BUTTONS AddFoldOutListItemButtons()
    {
        ...
        bool upPressed = GUILayout.Button(new GUIContent(upArrow, "上移"),
                                          EditorStyles.toolbarButton);

        // The down arrow will move things towards the end of the List
        var dnArrow = '\u25BC'.ToString();
        bool downPressed = GUILayout.Button(new GUIContent(dnArrow, "下移"),
                                            EditorStyles.toolbarButton);
        ...
        bool removePressed = GUILayout.Button(new GUIContent("-", "移除"),
                                              EditorStyles.toolbarButton);

        // Add button - Process presses later
        bool addPressed = GUILayout.Button(new GUIContent("+", "插入新的"),
                                           EditorStyles.toolbarButton);
...
public static class YBO_Editor_Extensions {

    public static T Cast<T>(this object o) where T : UnityEngine.PropertyAttribute
    {
        return (T)o;
    }
